...
$(document).ready(function(){
    ...
    smartwork.loadDataDict('SEND_SERVICE_TYPE'
    ,function(array){
    ...
    var modifyIfConfigFn = function(rowIndex) {
        ...
        var submitFn = function() {
            ...
            //提交
            smartwork.ajax({
                url : '../config/UpdateSendIfConfig.do'
                ,'show-loading': true
                ,data: smartwork.prefixName(values, 'bean.')
            }, function(result){
                dialog.modal('hide'); //隐藏
    ...
    var copyIfConfigFn = function() {
        ...
        var submitFn = function() {
            ...
            //提交
            smartwork.ajax({
                url : '../config/CopySendIfConfig.do'
                ,'show-loading': true
                ,data: values
            }, function(result){
                dialog.modal('hide'); //隐藏
    ...
    var addQueryConfigFn = function() {
        ...
        //设置标题
        dialog.find('.modal-title').text('新增发送配置');
    ...
    var modifyQueryConfigFn = function(rowIndex) {
        ...
        //设置标题
        dialog.find('.modal-title').text('修改发送配置');
    ...
    var deleteQueryConfigFn = function(rowIndex) {
        ...
        if (null == datas || datas.length <= 0) {
            smartwork.error('请选择要删除的发送配置');
            return;
        }
         
        smartwork.confirm('确定要删除选择的发送配置？<span class="text-danger">删除会导致对应的所有子配置被删除。</span>', function(action){ 
    ...
    var copyQueryConfigFn = function() {
        ...
        //设置标题
        dialog.find('.modal-title').text('复制发送配置');
