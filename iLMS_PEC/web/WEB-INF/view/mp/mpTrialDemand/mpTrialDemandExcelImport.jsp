...
function downloadTemplateFn(){
	var downurl = encodeURI(encodeURI(__ctx + '/comm/comm/downloadExcelModel?fileName=需求计算导入模板.xlsx'));
		...
		success : function(responseText) {
			...
			if (resultMessage.isSuccess()) {
				...
				$.topCall.confirm("提示",showMsg,
					function(r) {
						if (!r) return;
						$('#importForm').attr("action", __ctx + "/mp/mpTrialDemand/importMpTrialDemandModel");
						$('#importForm').submit();
					}
				);
			}
		...
		success : function(responseText) {
			...
			if (resultMessage.isSuccess()) {
				...
				$.topCall.confirm("提示",showMsg,
					function(r) {
						if (!r) return;
						$('#importForm').attr("action", __ctx + "/mp/mpTrialDemand/importMpTrialDemandModel2");
						$('#importForm').submit();
					}
				);
			}
...
function isImportFn(){
	...
	var url=__ctx + "/mp/mpTrialDemand/isImport";
...
function downloadDataFn(){
	var downurl = encodeURI(__ctx + '/mp/mpTrialDemand/exportTempData');
	$('#downloadiframe').attr('src', downurl);
}
	
function loadGrid() {
	$('#grid').datagrid($.extend($defaultOptions,{
		url :  __ctx + "/mp/mpTrialDemand/curdlistImportTempJson",
		idField : "id",
		sortName : 'ID',
		sortOrder : 'desc',
		columns : [ [
		...
		{field : 'arriveTimeStr',sortName : "ARRIVE_TIME",title : '到货日期',width : 130,align : 'center',sortable : 'true'}, 
		{field : 'excOrderNum',sortName : "EXC_ORDER_NUM",title : '例外需求',width : 130,align : 'center',sortable : 'true'}, 
