<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="java.util.List,com.hotent.base.core.util.AppUtil" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%
	String basePath = request.getContextPath();
	// 在flex中保存，发布流程的请求地址
	String postUrl = basePath + "/flow/def/flexDefSave";
	String flowKeyGetUrl = basePath+"/flow/def/getFlowKey";
	
	String flowListGetUrl = basePath+"/flow/def/getFlowListByTypeId";
    String xmlRecord = request.getAttribute("xmlRecord").toString();
	// 加载数据操作 
	// 获取流程定义id
	String loadDateUrl = ""; // flex中获取加载数据的地址
	String defId = request.getAttribute("defId").toString(); 
	if(defId != null && !defId.equals("")){
		loadDateUrl = basePath + "/flow/def/flexGet?defId=" + defId;
	}
%>
<!DOCTYPE html>
  <head>
  	<title>
  		在线流程设计--<c:choose>
  			<c:when test="${not empty bpmDefinition}">
  				${bpmDefinition.name} --(版本${bpmDefinition.version})
  			</c:when>
  			<c:otherwise>
  				${name}
  			</c:otherwise>
