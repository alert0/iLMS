            ...
            function fetchModel(modelId) {
                ...
                $http({method: 'GET', url: modelUrl}).
                    success(function (data, status, headers, config) {
                		...
                		if(childShapes && childShapes.length){
            ...
            $rootScope.$on('$includeContentLoaded', function (event) {
	            if (!$rootScope.editorInitialized) {
	                ...
	                jQuery(window).resize(function () {
	                    ...
	                    var totalAvailable = jQuery(window).height() - offset.top - mainHeader.height() - 43;
	                    canvas.height(totalAvailable - propSectionHeight);
	                    jQuery('#paletteSection').height(totalAvailable);
	                    jQuery('#propertiesHelpWrapper').height(totalAvailable);
	                    jQuery('#canvasHelpWrapper').height(totalAvailable);
	                    jQuery('#canvasSection').height(totalAvailable);
