	...
	Plugin.prototype = {
		...
		initUI : function() {
			...
			$(document).on('click','a[data-toggle="tab"]',function (e) {
			 ...
			 // TODO: 新的初始化操作,且只能在这里执行，避免多次绑定事件
	        me.jQInit();
		...
		parserTemplate : function(el) {
			...
			var url=__ctx + "/portal/sysIndexColumn/sysIndexColumn/getData.ht";
			...
			$.ajax({
				...
				success : function(data) {
					...
					if($('#newsBody_can').find('li.active').find('tbody').find('tr').length <2){
						$("#newsTit_can>a.active_setion").trigger('click');
					}
					me.parseTarget(wigetObj);
				},
				error : function(data) {
					me.unblockUI(this);
				}
		...
		parser : function() {
			me.parseTarget($('.index-page'));
			setTimeout(function(){
				var parentIframe = $(parent.document).find(".iframeWrap").children().get(0);
		...
		parseTarget : function(obj) {
			var indexPageTemplate = obj.find("[" + me._defaults.templateAliasKey + "]");
	    ...
	    ,jQInit: function () {
	        $(function () {
	        	...
	        	$(document).on('click', '.span', function (e) {
					var $this = $(this);
					var $parent = $this.parent('.line--row__head--subTit');
	                ...
	                // ul 切换
	                var $ul = $parent.siblings('.line--row__head--list').find('ul');
	                var len = $ul.length;
	                if (len <= 1){
	                    return;
	                }

	                // var index = $('.span', _this).index($this);
	                var thisIndex = $parent.find('> .span').index($this);		// active index
	                $ul.eq(thisIndex).show();
	                $ul.eq(thisIndex).siblings().hide();

	                // 更多切换
	                var $more = $this.siblings('.tit--more');
	                if ($more.length > 0) {
	                    $more.hide().eq(thisIndex).show();
	                }
	            });

	            // 自定义的打开链接
	            $(document).on('click', '[name="addTab"]', function (e) {
	                /**
					 * 首页统计栏目，点击栏目跳转操作
	                 * @type {*|jQuery|HTMLElement} <div class="appBar__contain--row appBar__bg--list color-c2c39f" data-name="appBar_list" name="addTab" type="tab" data-url="${data.oaToDoUrl}" data-txt="OA待办"> html </div>
					 * [name="addTab"]: 必填
					 * type: [tab: 打开tab, blank:打开新标签]
					 * data-url: url
					 * data-txt: title
	                 */
	                var $this = $(e.target);

	                if (!$this.hasClass('appBar__contain--row')) {
	                    $this = $this.parents('.appBar__contain--row');
	                }

	                // var alias = $this.attr('template-alias');	// 栏目别名，自带参数
	                var menuOption = $this.data();				// data-url="" data-txt=""
	                var type = $this.attr('type');				// type="tab" || type="blank"

	                if (!!type && type == 'tab'){
	                    // menuOption['id'] = 'poly_'+ alias; 		// 'poly_'+ alias
	                    // me.addToTab(menuOption.url, menuOption.txt, menuOption.id, '');
	                    me.addParentTab(menuOption.url, menuOption.txt);
	                }else {
	                    parent.window.open(__ctx +menuOption.url, "_blank");
	                }
	            }).on('click', '[name="liAddTab"]', function (e) {
	                /**
	                 * 列表的跳转
	                 * @type {*|jQuery|HTMLElement} <li class="li" name="liAddTab" type="tab" data-url="${data.alreadyDoUrl}" data-txt="OA已办"> xxx </li>
	                 * [name="liAddTab"]: 必填
	                 * type: [tab: 打开tab, blank: 打开新标签]
					 * data-url: url
					 * data-txt: title
	                ...
	                if (!!type && type == 'tab'){
	                    if (!!data.rtnStatus) {
	                    	// TODO: OA单独处理 参考 view/office/receivedProcess/pending.jsp
							if (data.rtnStatus == 0) {
	                            // $.topCall.success("此任务已经被其他人处理完成!");
	                            $.messager.alert('提示', '此任务已经被其他人处理完成!', 'success');
							}else if (data.rtnStatus == 4) {
	                            // $.topCall.success("此任务已经被其他人锁定!");
	                            $.messager.alert('提示', '此任务已经被其他人锁定!', 'success');
							}else if (data.rtnStatus == 6) {
	                            // $.topCall.error("流程已经被禁止，请联系管理员！");
	                            $.messager.alert('提示', '流程已经被禁止，请联系管理员！', 'error');
							}else {
	                            $.openFullWindow(__ctx +data.url);
							}
						}else {
	                        me.addParentTab(data.url, data.txt);
						}
	                }else {
	                    parent.window.open(__ctx +data.url, "_blank");
	                }
	            }).on('click', '[name="liMore"]', function (e) {
	                /**
					 * 点击 更多》打开新的tab
	                 * @type {*|jQuery|HTMLElement} <span class="tit--more" name="liMore" type="tab" data-url="${data.moreUrl}" data-txt="${model.title}">更多 》</span>
					 * [name="liMore"]: 必填
					 * type: tab
					 * data-url: url
					 * data-txt: title
	            ...
	            }).on('click', '.square__row', function (e) {
	                /**
					 * 常用工具链接跳转及弹窗
	                 * @type {*|jQuery|HTMLElement} <div class="square__row ${item.icon}" type="${item.jumpType}" data-url="${item.url}" data-txt="${item.name}" data-layer> <p>${item.name}</p> </div>
					 * type: [1: 打开tab, 2: 打开新标签, 3: 弹窗]
					 * data-url: url
					 * [data-txt]: title, 打开tab时需要此属性。亦或让type = 3
					 * [data-layer]: 若需要弹窗则添加此属性
	                 */
					var $this = $(e.target);

					if (!$this.hasClass('square__row')) {
						$this = $this.parents('.square__row');
					}
	                ...
	                if (data.url == '') {
	                    $.messager.alert('提示', 'URL 参数配置错误', 'info');
	                    return false;
	                }
	                me.addParentTab(data.url, data.txt);
	            });
	    ...
	    ,addParentTab: function (url, txt, id) {
			var $tab = parent.$('#tabs');
			...
			if (!$tab.tabs('exists', txt)) {
				var content = '<iframe scrolling="auto" frameborder="0" src="'+ __ctx + url +'" style="width:100%;height:100%;"></iframe>';
	            $tab.tabs('add', {
	                title: txt,
	                content: content,
	                iconCls: '',
