...
Permission=function(){
	...
	this.load=function(url,params){
		...
		var _self=this;
//		$.topCall.progress({type:"progress", title:'请等待', msg:'正在加载表单权限,请稍后...'});
		$.post(url, params,function(data){
//			$.topCall.progress({type:"closeProgress"});
			...
			if(fields!=undefined && fields!=''){
				...
				var fieldHtml=_self.getPermission(_self.FieldsPermission,"field",true);
			...
			if(tables!=undefined && tables!=''){
				...
				var tableHtml=_self.getPermission(_self.SubTablePermission,"subtable",true);
			...
			if(opinions!=undefined && opinions!=''){
				...
				var opinionHtml=_self.getPermission(_self.Opinion,"opinion",true);
	...
	this.handChange=function(){
		...
		$("#fieldPermission,#tablePermission,#opinionPermission").delegate("select.r_select,select.w_select,select.b_select","change",function(){
			...
			var formFieldId=trObj.attr("formFieldId");
			...
			var selType=obj.val();
			_self.changePermission(permissionType,idx,mode,selType,"","",formFieldId);
	...
	this.getPermissionByFormFieldId = function(aryPermission,formFieldId){
		for(var i =0 ,c;c=aryPermission[i++];){
			if(c.subFieldJsonList&&c.subFieldJsonList.length>0){
				return this.getPermissionByFormFieldId(c.subFieldJsonList,formFieldId);
			}else if(c.formFieldId==formFieldId){
				return c;
			}
		}
	};
	 ...
	 */
	this.changePermission=function(permissionType,idx,mode,type,ids,names,formFieldId){
		if(idx==-1) return;
		var aryPermission=[];
		...
		var objPermssion=this.getPermissionByFormFieldId(aryPermission,formFieldId);
	 ...
	 */
	this.getPermission=function(aryPermission,type,isFirst){
		...
		if(type=='subtable'){
			...
			//	subTableHtml += '<tr><td colspan="4">'+tablePermission.memo+'</td></tr>';
				...
				for(var j=0;j<fieldPermission.length;j++){
					...
					var str=this.getHtml(objPermission, type,isFirst);
					subTableHtml += str;
				}
	 ...
	 */
	this.getHtml=function(permission,type,isField){
		var _self=this;
		if(permission.subFieldJsonList&&permission.subFieldJsonList.length>0){
			return "<tr><td colspan='5'>"+""+"</td></tr><tr><td colspan='5' style='background-color: #CACACA;color: rgb(88, 88, 100);font-weight: bold;'>复合字段-"+permission.memo+"</td></tr>"+ _self.getPermission(permission.subFieldJsonList,"field",false)+"<tr><td colspan='5'>"+""+"</td></tr>";
		}else if (permission.gourpFiledList&&permission.gourpFiledList.length>0){
			return "<tr><td colspan='5'>"+""+"</td></tr><tr><td colspan='5' style='background-color: #CACACA;color: rgb(88, 88, 100);font-weight: bold;'>分组字段-"+permission.memo+"</td></tr>"+ _self.getPermission(permission.gourpFiledList,"field",false)+"<tr><td colspan='5'>"+""+"</td></tr>";
		}else{
			return _self.getFieldHtml(permission,type,isField);
		}
	};
	
	this.getFieldHtml = function (permission,type,isField) {
		var rpostTd ="",firstTr="";
		if(type=='field'){
			var f =permission.rpost?' checked="checked" ':''; 
			rpostTd =  '<td onclick="toggleCheck(this);"><input name="rpost" type="checkbox" '+f+'/></td>';	
		}
//		if(isField) firstTr="<tr><td colspan='5' style='background-color: #CACACA;color: rgb(88, 88, 100);font-weight: bold;'>"+"主字段"+"</td></tr>";
		var aryTr = [firstTr,'<tr style="height: 53px;" type="#permissionType" formFieldId="'+permission.formFieldId+'" tableId="'+permission.tableId+'" mainTableId="'+permission.mainTableId+'" tableName="'+permission.tableName+'" mainTableName="'+permission.mainTableName+'">'
			, '<td>#desc</td>'
			//只读
	...
	this.getHtmlTd=function(v,full,type){
		...
		var tdstr = ' colspan="3" ng-click="alert()" ng-show="'+type+'.'+full+'"';
		if(type=="subtable"){
			tdstr += ' width="25%" ';
		}
		var aryTd = ['<td'+tdstr+'>'
			, '<select class="form-control ',v,'_select"  permissonType="#',v,'_type" name="#name"  >'
	...
	this.addOpinion=function(formName,name){
		...
		//意见权限。
		var opinionHtml=this.getPermission(this.Opinion,"opinion",true);
	...
	this.readPermissionByObj=function(scope,type){
		...
		$("tr",objScope).each(function(index){
			...
			var permission={
					...
					"tableName":tableName,"mainTableName":mainTableName,"formFieldId":formFieldId,
