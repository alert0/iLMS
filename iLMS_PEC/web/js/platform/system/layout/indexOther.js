	...
	scope.getUserRes = function(){
		baseService.post(__ctx+"/base/base/sysResource/getResTree",{}).then(
			function(data){
                ...
                setTimeout(function () {
                    $.parser.parse($('#mh-mainMenu'));	// TODO: 重新渲染easyUI 菜单组件
                },800);
...
$(function(){
	...
	//TODO: easyUI 重新渲染之后导致angular上下文丢失，重新绑定click事件
	$(document).on('click', '[name="data-add-tab"]', function (e) {
		var $this = $(e.target);
		if ($this.is('[data-name="mh-menu-item"]')) {
			$this = $this.find('> .menu-text > span');
		}else if ($this.is('.menu-text')) {
			$this = $this.find('> span');
		}

		var menuOption = $this.data();
		addTab(menuOption);
    });
});
...
var tabs = $('#tabs'),
	...
	curTabUrl = $(currentTab.children('iframe')[0]).attr('src');
