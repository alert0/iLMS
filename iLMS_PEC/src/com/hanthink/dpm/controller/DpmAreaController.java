...
public class DpmAreaController extends GenericController{
		...
		try {
			if(StringUtil.isEmpty(id)){
				...
				if (d!=null) {
					...
					dpmAreaModel.setCreationUser(user.getAccount()); //记录创建人
					dpmAreaManager.create(dpmAreaModel);
					resultMsg="保存成功";
				}
			}else{
				dpmAreaManager.updateAndLog(dpmAreaModel, RequestUtil.getIpAddr(request));
				dpmAreaModel.setLastModifiedUser(user.getAccount());//记录修改人
				resultMsg="修改成功";
			}
	...
	public @ResponseBody List<DictVO> getUnloadCode(HttpServletRequest request,HttpServletResponse reponse) throws Exception{
		try {
	...
	public @ResponseBody  List<DictVO> getUnloadWareCode(HttpServletRequest request,HttpServletResponse reponse) throws Exception{
		try {
			List<DictVO> list = new ArrayList<DictVO>();
//			StringBuilder strWareName = new StringBuilder();
            List<DictVO> models = dpmAreaManager.getUnloadWareCode();
            for (DictVO dictVO : models) {
            	StringBuilder strWareName = new StringBuilder();
            	//如果仓库名称不为空则拼接
            	if (StringUtil.isNotEmpty(dictVO.getValueName())) {
            		strWareName.append(dictVO.getValueKey()+"("+dictVO.getValueName()+")");
				}
				dictVO.setValueName(strWareName.toString());
				list.add(dictVO);
	...
	public @ResponseBody List<DictVO> getUnloadWorkcenter(HttpServletRequest request,HttpServletResponse reponse) throws Exception{
		IUser user = ContextUtil.getCurrentUser();
	...
	public @ResponseBody DpmAreaModel getWareNameByCode(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		DpmAreaModel model = dpmAreaManager.getWareNameByCode(wareCode);
		try {
