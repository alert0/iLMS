...
public class DpmDepPersonController extends GenericController{
	 ...
	 */
	@RequestMapping("save")
	public void save(HttpServletRequest request,HttpServletResponse response, DpmDepPersonModel dpmDepPersonModel) throws Exception{
		String resultMsg = null;
		String id = dpmDepPersonModel.getId();
		IUser user = ContextUtil.getCurrentUser();
		try {
			if(StringUtil.isEmpty(id)){
				dpmDepPersonModel.setFactoryCode(user.getCurFactoryCode());
				dpmDepPersonModel.setCreationUser(user.getAccount());
				if (!StringUtil.isEmpty(dpmDepPersonModel.getAccount())) {
					boolean b = dpmDepPersonManager.isUserIdExist(dpmDepPersonModel);
					if (b) {
						resultMsg="人员已存在，请重新选择";
						writeResultMessage(response.getWriter(),resultMsg,ResultMessage.FAIL);
						return;
					}
//					boolean isTrue = dpmDepPersonManager.isDepExixt(dpmDepPersonModel);
//					if (isTrue) {
//						resultMsg="责任组已存在，请重新选择";
//						writeResultMessage(response.getWriter(),resultMsg,ResultMessage.FAIL);
//						return;
//					}
				}
				...
				dpmDepPersonModel.setLastModifiedUser(user.getAccount());
