...
public class PkgProposalModel extends AbstractModel<String>{
	...
	/**邮件发送标识 1:不需要发送（已发送）**/
	public static final String EMAIL_FLAG_YES = "1";
	/**
	 * 
	 */
	...
	/**显示在变更界面,不显示在提案界面 1**/
	public static String IS_SHOW_CHANGE_YES = "1";
	
	/**显示在提案界面，不显示在变更界面 0**/
	public static String IS_SHOW_CHANGE_NO = "0";
	
	/**在提案和变更界面都显示 2**/
	public static String IS_SHOW_CHANGE_ALL = "2";
    ...
    /**************************************************************************/
    /**物流担当**/
    private String partRespUserName;
	...
	public static void checkImportData(PkgProposalModel model)throws Exception {
		String reg = "^[0-9]*[1-9][0-9]*$";
		StringBuffer checkInfo = new StringBuffer();
		
		if (null != model.getBoxRequireQty() && StringUtil.isNotEmpty(model.getBoxRequireQty().trim())) {
		if (!Pattern.matches(reg, model.getBoxRequireQty().trim())) {
			checkInfo.append("箱子需求量应为正数");
		}
		}
		
		if ( null != model.getTrayRequireQty() && StringUtil.isNotEmpty(model.getTrayRequireQty().trim())) {
			if (!Pattern.matches(reg, model.getTrayRequireQty().trim())) {
				checkInfo.append("托盘需求量应为正数");
			}
		}
	...
	public void setIsShowChange(String isShowChange) {
	...
	public String getPartRespUserName() {
		return partRespUserName;
	}

	public void setPartRespUserName(String partRespUserName) {
		this.partRespUserName = partRespUserName;
	}
