...
public class SwNonStandardController extends GenericController {
	...
	public void uploadSignProFile(HttpServletRequest request, HttpServletResponse response , SwNonStandardModel model) throws Exception {
		try {
	...
	public @ResponseBody List<SwNonStandardModel> getUploadPicIdByModel(SwNonStandardModel model ,HttpServletRequest request,
			HttpServletResponse reponse) throws Exception {
		try {
...
//			String imgPath = request.getRealPath("/css/img/");
...
//					String imgName = list.get(i).getSaleNo() + list.get(i).getPartShortNo() + i + picSuffix;
//					String tagImgPath = imgLastPath + imgName; //图片下载到本地路径
//					
//					nse.saveToFile(list.get(i).getFeatureValue(), tagImgPath);
//					String tagUrl = "http://"  + ipAddr + ":" + port + "/css/img/" + fileName+ "/" +imgName;
...
//			}
	 ...
	 * @throws  
	 * @author luoxq
	 * @date 2019年3月15日 下午12:06:37
	 */
	@RequestMapping("/getCheckHistory")
	public @ResponseBody List<SwNonStandardModel> getCheckHistory(SwNonStandardModel model ,HttpServletRequest request,
			HttpServletResponse reponse) throws Exception {
		try {

			List<SwNonStandardModel> list = (List<SwNonStandardModel>) manager.getCheckHistory(model);
			return list;
		} catch (Exception e) {
			e.printStackTrace();
			log.error(e.toString());
	...
	public void submitFeedback(HttpServletRequest request, HttpServletResponse reponse, 
			SwNonStandardModel model)
			throws Exception {
		ResultMessage message = null;
		try {
			IUser user = ContextUtil.getCurrentUser();
			String creationUserIp = RequestUtil.getIpAddr(request);
			model.setCreationUserIp(creationUserIp);
...
//		message = new ResultMessage(ResultMessage.FAIL, "发布失败");
	...
	private void exportNonStandDetail(SwNonStandardModel model, HttpServletRequest request, HttpServletResponse response) {
		...
		String exportFileName = "定制化信息【" + model.getSaleNo() + "-" + model.getSaleRowNo() + "-" +model.getPartNo() + "】" + ExcelUtil.EXCEL_XLSX;
...
//	@SuppressWarnings("resource")
	...
	private void exportMultProposalFile( HttpServletRequest request, HttpServletResponse response) {
		...
		for(String modelArr : models){
			...
			//文件打包
			String tmpFileName = "定制化信息【"  + model.getSaleNo() + "-" + model.getSaleRowNo() + "-" +model.getPartNo() +  "】" + ExcelUtil.EXCEL_XLSX;
			try {
