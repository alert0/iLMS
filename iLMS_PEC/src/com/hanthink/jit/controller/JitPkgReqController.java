...
public class JitPkgReqController extends GenericController{
	...
	public void downloadJitPkgReqModel(HttpServletRequest request,HttpServletResponse response,
		...
		try {
			...
			String[] headers = {"车间","信息点", "零件号","简号",
					"零件名称", "拣货工程", "台车", "配送工程",
					"配送地址", "配送量", "收容数","订购包装数", "箱数",
					"出货仓库", "到货仓库", "下线批次进度",
					"备件批次进度","发货批次进度", "到货批次进度", "配送批次进度",
					"供应商代码", "出货地代码","供应商名称", "物流单号",
					"创建时间"};
			String[] columns = {"workcenter","planCodeDesc", "partNo","partShortNo",
					"partName", "preparePerson", "carpool", "distriPerson",
					"location", "requireNum", "standardPackage","distriPackage", "xs",
					"shipDepot", "arrDepot", "kbProductSeqno",
					"prepareProductSeqno", "deliveryProductSeqno", "arriveProductSeqno","distriProductSeqno",
					"supplierNo", "supFactory","supplierName", "orderNo",
					"creationTime"};
			int[] widths = {60, 80, 100, 80,
					80, 80, 60, 80, 
					80, 70, 80, 80, 60,
				    80, 80, 80,
				    100, 100, 100, 100,
			...
			ExcelUtil.exportExcel(ExcelUtil.EXCEL_XLSX, request, response, "拉动订购需求明细查询"+df.format(new Date()), list, headers, widths, columns);
		} catch (Exception e) {
			e.printStackTrace();
