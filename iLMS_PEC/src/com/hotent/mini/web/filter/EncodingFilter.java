...
//      response.setHeader("Access-Control-Allow-Headers","x-requested-with, accept, origin, content-type"); 
        response.setHeader("Access-Control-Allow-Headers","x-requested-with, token, Authorization, accsess-token, accept, content-type,User-Agent,Cookie"); 
		response.setHeader("Access-Control-Max-Age", "3628800");
		response.setHeader("Access-Control-Allow-Credentials","true"); 
		response.setHeader("Access-Control-Allow-Methods","POST,GET,PUT,DELETE,OPTIONS");
		String method = request.getMethod();
		if (null != method && "OPTIONS".equals(method.toUpperCase())) {
			return;
		}
