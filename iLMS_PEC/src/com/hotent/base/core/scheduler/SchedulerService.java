...
public class SchedulerService {
	...
	public ResultMessage addJob(String jobName, String className,
			...
			throws SchedulerException {
		if (scheduler == null)
			return new ResultMessage(ResultMessage.FAIL, "scheduler 没有配置!");
		boolean isJobExist = this.isJobExists(jobName);
		if (isJobExist) {
			return new ResultMessage(ResultMessage.FAIL, "任务已存在");
		}
		Class<Job> cls;
		try {
	 ...
	 */
	@SuppressWarnings({ "unchecked" })
	public ResultMessage addJob(String jobName, Class<?> cls,
			Map<String, Object> parameterMap, String description) {
		if (scheduler == null)
			return new ResultMessage(ResultMessage.FAIL, "scheduler 没有配置!");
	...
	private void setTrigBuilder(String planJson, TriggerBuilder<Trigger> tb)
		...
		case PlanObject.TYPE_FIRST:
			...
			tb.startAt(date).withDescription("执行一次,执行时间:" + date.toString());
			break;
		// 每分钟执行
	 ...
	 * @param job
	 */
	private void setJobMap(String json, JobBuilder job) {
		JSONArray aryJson = JSONArray.fromObject(json);
		ParameterObj[] list = (ParameterObj[]) JSONArray.toArray(aryJson,
				ParameterObj.class);
