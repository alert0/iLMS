...
<script type="text/javascript">
	...
	app.controller("PreviewUserController", function($scope, $http, $jsonToFormData) {
		...
		$scope.selectUsers = function(str) {
			var callBack=function(data, dialog) {
				...
				$scope.$apply(function() {
					eval("$scope."+str+"=data[0].id;");
					eval("$scope."+str+"Name=data[0].name;");
				});
			};
		...
		$scope.selectGroups = function(str) {
			new GroupSelectDialog({
				type : '1',
				single : true,
				callback : function(groupIds, names, records, dialog) {
					$scope.$apply(function() {
...
<body ng-controller="PreviewUserController">
	<div class="buttons">
		<a href="javaScript:void(0)" class="btn btn-sm btn-primary fa-save" ng-click="previewCondition()">
			<span>预览</span>
		</a>
		<a href="javaScript:void(0)" onclick="javascript:window.selfDialog.dialog('close')" class="btn btn-sm  btn-primary fa-close">
			<span>关闭</span>
		</a>
	</div>
	<div id="preVerwTab" class="easyui-tabs" fit="true">
		<div title="预览参数" style="padding: 20px;">
			<form id="paramForm">
				<table class="table-form" cellspacing="0">
					...
					<tr ng-show="variables.startUser">
						<th>
							<span>发起人:</span>
							<span class="required">*</span>
						</th>
		...
		<div title="查询结果">
			<div style="height: 100%; width: 100%">
				<table id="userGridList" data-options="fitColumns:false,checkOnSelect:true,pagination:true,autoRowHeight:false" url="${ctx}/flow/node/previewCondition">
					<thead>
						<tr>
