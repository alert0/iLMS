<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@include file="/commons/include/html_doctype.html" %>
<html ng-app="app">
	<head>
		<%@include file="/commons/include/ngEdit.jsp" %>
		<script type="text/javascript" src="${ctx}/js/bpmx/test/bpmTestCase/bpmTestCaseGetController.js"></script>
	</head>
	<body class="easyui-layout" ng-controller="ctrl">
		<!-- 顶部按钮 -->
		<div class="toolbar-panel">
			<div class="buttons">
				<a class="btn btn-primary fa-back" ng-click="closeFrame();"><span>返回</span></a>
			</div>
		</div>
		<form name="form" ht-load="getJson?id=${param.id}"   ng-model="data">
		<table class="table-form"   cellspacing="0">
					<tr>								
						<th>测试用例名称:</th>
						<td> {{data.name }}</td>
					</tr>
					<tr>								
						<th>表单数据:</th>
						<td> {{data.boFormData }}</td>
					</tr>
					<tr>								
						<th>流程变量:</th>
						<td> {{data.flowVars }}</td>
					</tr>
					<tr>								
						<th>发起人:</th>
						<td> {{data.startor }}</td>
					</tr>
					<tr>								
						<th>审批动作， 默认动作时 agree:</th>
						<td> {{data.actionType }}</td>
					</tr>
					<tr>								
						<th>断点测试设置:</th>
						<td> {{data.bpmDebugger }}</td>
					</tr>
		</table>
		
		
		</form>
	</body>
</html>