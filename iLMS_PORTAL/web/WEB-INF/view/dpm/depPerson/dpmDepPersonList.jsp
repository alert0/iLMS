		...
		<div id="gridSearch" class="toolbar-search ">
			<div class="toolbar-head">
				...
				<div class="buttons">
					...
					<a class="btn btn-sm btn-primary fa-add" id="add"  onclick="openDetail('','add')">
					 ...
					 <a class="btn btn-sm btn-primary fa-remove"  action="/dpm/depPerson/remove">
						<span>删除</span>
					</a>
			...
			<div class="toolbar-body">
				<form id="searchForm" class="search-form">
					<ul>
						<li><span>用户名:</span>
						<input  class="inputText" name="account">
						</li>
						<li><span>姓名:</span>
						<input class="inputText" name="fullname">
						</li>
					    <li><span>所属科室:</span>
					    <input  class="inputText" type="text" name="belongDep">
					    </li>
					    <li><span>责任组:</span>
					    <select id="seaFormDepCode" class="inputText" type="text" name="depCode"></select>
					    </li>
					</ul>
				</form>
			</div>
...
<script>
	...
	function openDetail(id, action) {
		...
		var url = __ctx + "/dpm/depPerson/dpmDepPerson" + action + "";
		if(!$.isEmpty(id)){
			url+='?id=' + id ;
		}
	 ...
	 function loadGrid() {
		$('#grid').datagrid($.extend($defaultOptions,{
			url :  __ctx + "/dpm/depPerson/curdlistJson",
			idField : "id",
			sortName : 'ID',
			sortOrder : 'desc',
			columns : [ [
			{field : 'id',sortName : "id",checkbox : true},     
			{field : 'account',sortName : "account",title : '用户名',width : 130,align : 'center',sortable : 'true'}, 
			{field : 'fullname',sortName : "areaCode",title : '姓名',width : 130,align : 'center',sortable : 'true'},
			...
			{field : 'defaultDiscoArea',sortName : "defaultDisco",title : '默认发现区域',width : 130,align : 'center',sortable : 'true'}, 
			{
				field : 'colManage',
				title : '操作',
				align : 'center',
				formatter : function(value, row, index) {
					...
					result += "<a class='rowInLine btn btn-default fa fa-remove' action='/dpm/depPerson/remove?id=" + row.id + "' herf='javascript:void(0)'>删除</a>";
