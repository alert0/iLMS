							...
							<c:forEach items="${subsystemList}" var="system">
								<c:if test="${!(system.id eq currentSystem.id)}">
					                <div onclick="changeSystem('${system.id}')">
					                	<a  href="javascript:;" class="btn btn-xs"  title="切换系统">${system.name}</a>
					                </div>
					         	</c:if>
							</c:forEach>
			            </div>
							...
							<c:forEach items="${orgList}" var="org">
								<c:if test="${!(org.id eq currentOrg.id)}">
					                <div onclick="changeOrg('${org.id}')" >
					                	<a  href="javascript:;" class="btn btn-xs" title="切换组织">${org.name}</a>
					                </div>
						         </c:if>
							</c:forEach>
			            </div>
				...
				<div data-options="iconCls:'fa fa-server'">
						...
						<c:if test="${not empty layoutManageList}">
							...
							<c:if test="${flagUserSetting}">
								<div data-options="iconCls:'fa fa-check'" style="cursor:not-allowed;"><span>我的布局</span></div>
							</c:if>
							<c:forEach items="${layoutManageList}" var="layout">
								<c:if test="${(layout.id eq userSetting.layoutId)}">
									<div data-options="iconCls:'fa fa-check'" style="cursor:not-allowed;"><span>${layout.name}</span></div>
								</c:if>
								<c:if test="${!(layout.id eq userSetting.layoutId)}">
					                <div onclick="changeLayout('${layout.id}')">
					                	<a  href="javascript:;" class="btn btn-xs"  title="切换布局">${layout.name}</a>
					                </div>
						         </c:if>
							</c:forEach>
				            </div>
				...
				<div data-options="iconCls:'fa fa-server'">
					...
					<div>
		                <div onclick="changeSkin('default')">
		                	<a  href="javascript:;" class="btn btn-xs"  title="切换皮肤">默认红</a>
		                </div>
		                <div onclick="changeSkin('blue')">
		                	<a  href="javascript:;" class="btn btn-xs"  title="切换皮肤">测试蓝</a>
		                </div>
			        </div>
				</div>
