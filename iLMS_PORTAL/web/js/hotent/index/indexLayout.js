...
//			}
		...
		initSetting:function(){
			...
			$('.sidebar-nav').css("min-height", $(window).height()-10);
		...
		saveRemoteLayout:function () {
			...
			$.ajax({
				type : "POST",
				url : __ctx+"/portal/sysIndexLayoutManage/sysIndexLayoutManage/saveLayout.ht",
				data : {
					id:$('#id').val(),
					name:$('#name').val(),
					memo:$('#memo').val(),
					...
					isDef:isDef,
					html : $.base64.encode(html.html(),"utf-8"),
					designHtml:	$.base64.encode(designHtml.html(),"utf-8")
				},
				success : function(data) {
					...
					if (obj.result == 1) {
						BootstrapDialog.confirm("保存成功,是否继续操作？", function(rtn){
							if(rtn){
									window.location.href=__ctx+"/portal/sysIndexLayoutManage/sysIndexLayoutManage/design?id="+obj.message;
							}else{
								if(window.opener){
									try{
						...
						$("#saveRemoteLayout").removeAttr("disabled");
					}
		...
		saveRemoteMyLayout:function(){
			...
			$.ajax({
				type : "POST",
				url : __ctx+"/portal/sysIndexMyLayout/saveLayout.ht",
				data : {
					html :  $.base64.encode(html.html(),"utf-8"),
					designHtml:$.base64.encode(designHtml.html(),"utf-8")
				},
				success : function(data) {
					var obj = eval('(' + data + ')');
					if (obj.result == 1) {
						$.topCall.confirm("tishi", "保存成功,是否继续操作？", function(rtn){
							if(rtn){
									window.location.href=__ctx+"/portal/sysIndexMyLayout/design";
							}else{
								if(window.opener){
									try{
										window.opener.location.href=window.opener.location.href.getNewUrl();
						...
						$.topCall.error(obj.message,"提示信息");
					}
		...
		selectTemplate:function(){
			...
			BootstrapDialog.show({
		        ...
		        url:__ctx+"/platform/system/sysIndexLayoutManage/selectTemplate",
		        //回调函数
		...
		deleteLayout:function(id){
			...
			$.topCall.confirm("tis","该操作是删除自己的布局，首页的布局取默认的布局。是否删除当前设计的布局？", function(rtn){
				if(rtn){
					
					$.ajax({
						type : "POST",
						url : __ctx+"/portal/sysIndexMyLayout/deleteLayout.ht",
						data : {
							id : id
						},
						success : function(data) {
