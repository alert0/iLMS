...
.directive('htValidate', [ function() {
	return {
		...
		link : function(scope, element, attr, ctrl) {
			...
			if($(element).is(":disabled") || $(element).is(":hidden")){
				return;
			}
	...
	return {
		...
		link : function(scope, element, attr, ctrl) {
			element.on("click", function() {
				if(scope.btnDisable&&scope.btnDisable.save){return;}
...
.directive('htDic', function($injector){
	return {
		...
		link: function(scope, element, attrs) { 
			element.removeClass();
			scope.permission ="b";
			if(typeof(attrs.permission) != "undefined"){
			...
			scope.dicClean = function(){
				...
				if(window.FORM_TYPE_=='mobile'){
        			$(element).find('input').css('width','100%').css('float','none');
        		}
		...
		link: function (scope,element,attrs,ctrl){
    		...
    		scope.$watch('htUpload',  function(newValue, oldValue) {
    			...
    			if(!newValue){ scope.files=[];}
                   	...
                   	var array=eval("(" +newValue +")")
                   if(oldValue!=undefined){
                   		var jsonOldValue=eval("(" +oldValue +")");
                      for (var i = 0; i <jsonOldValue.length ; i++) {
                          array.push(jsonOldValue[i]);
                       }
				   }
                    scope.files = array;
    			}
...
.directive('htTimes', [ function() {
		return {
			...
			template:'<div ng-show="permission!=\'r\'"><input type="text" ng-model="day" ng-change="changeMin()" class="inputText" style="width: 80px;" placeholder="天数" ht-validate="{number:true}"/><label>天</label>\
						<select  ng-model="hour" ng-change="changeMin()" class="inputText" style="width: 80px;" ng-options="hour as (hour) for hour in hourArr">\
						</select><label>小时</label>\
						<select ng-model="minute" ng-change="changeMin()" class="inputText" style="width: 80px;" ng-options="minute as (minute) for minute in minuteArr">\
