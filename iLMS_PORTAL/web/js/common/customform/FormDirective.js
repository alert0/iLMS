			 ...
			 if(attr.htGanged){
				 ...
				 function gangedChangeFields(changeFields,id){
					 for(var i=0;i<changeFields.length;i++){
						 ...
						 switch(changeField.type){
						 	case "1": 	isGroup?gangedGroup(elm,false):fieldShowOrHide(elm,false); break;
				        	case "2": 	isGroup?gangedGroup(elm,true):fieldShowOrHide(elm,true); break;
				        	case "3": 	readOnlyElem(true,changeField,elm);break;
				        	case "4": 	readOnlyElem(false,changeField,elm); break;
				        	case "5": 	changeRequired(elm,true); break;
				        	case "6": 	changeRequired(elm,false); break;
				        	case "7": 	var tmp= isSub?'scope.'+modelStr+'=""':'scope.$parent.data.'+changeField.pathName+'=""';eval(tmp);cleanElem(changeField,elm); break;
				        	case "8": 	gangedCascade(id,changeField,1,elm); break;
						 }
					 }
				 }
				 ...
				 function readOnlyElem(isRead,field,elm){
					 switch(field.ctrlType){
					 	case "radio": 	isRead?$(elm).find("input[type=radio]").attr("disabled",true):$(elm).find("input[type=radio]").attr("disabled",false); break;
			        	case "checkbox": isRead?$(elm).find("input[type=checkbox]").attr("disabled",true):$(elm).find("input[type=checkbox]").attr("disabled",false); break;
			        	case "selector": 
			        	case "fileupload": 	isRead?$(elm).find("a[ng-click]").css("pointer-events","none"):$(elm).find("a[ng-click]").css("pointer-events","auto"); break;
			        	case "dic": 	isRead?$(elm).find("span").css("pointer-events","none"):$(elm).find("span").css("pointer-events","auto"); break;
			        	default: isRead?$(elm).attr("disabled","disabled"):$(elm).removeAttr("disabled"); break;
					 }
				 }
				 
				 function cleanElem(field,elm){
					 switch(field.ctrlType){
			        	case "checkbox": $(elm).find("input[type=checkbox]").attr("checked",false); break;
			        	case "select":  $(elm).val(null).trigger("change");break;
			        	default:  break;
					 }
				 }
				 ...
				 function gangedBack(id){
					 ...
					 for(var i=0;i<changeList.length;i++){
						 ...
						 if(change.id==id){
							 switch(change.type){
							 	case "1": 	isGroup?gangedGroup(elm,true):fieldShowOrHide(elm,true); break;
					        	case "2": 	isGroup?gangedGroup(elm,false):fieldShowOrHide(elm,false); break;
					        	case "3": 	readOnlyElem(false,change,elm);break; break;
					        	case "4": 	readOnlyElem(true,change,elm);break; break;
					        	case "5": 	changeRequired(elm,false); break;
					        	case "6": 	changeRequired(elm,true); break;
					        	case "7": 	var tmp= isSub?'scope.item.'+change.name+'="'+change.value+'"':'scope.$parent.data.'+change.pathName+'="'+change.value+'"';eval(tmp); break;
					        	case "8": 	gangedCascade(id,change,0); break;
							 }
							 delArray.push(i);
						 }
					 }
							 ...
							 for (var i = 0; i < options.length; i++) {
								 ...
								 for (var m = 0; m < reduce.length; m++) {
									if(option.value==reduce[m].id){
										...
										if(option.selected){
											var tmp= isSub?'scope.item.'+changeField.pathName+'=""':'scope.$parent.data.'+changeField.pathName+'=""';eval(tmp);
										}
										break;
									}
								 }
