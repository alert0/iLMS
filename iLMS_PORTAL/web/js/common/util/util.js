 ...
 * url : url, --必须
 * param : {}, --可选
 * valueKey : '', --可选 默认 valueKey
 * textKey : '', --可选 默认 valueName
 * dictArr:[{
		typeKey:'YES_NO', --可选 
		el : '#searchFormCol4', --必须
		defaultVal : '', --可选
		addBlank : true --可选
 * }]
 */
HtUtil.loadComboBox = function(obj){
	var typeKeyArr = [];
	obj.dictArr.forEach(function(dicType){
		typeKeyArr.push(dicType.typeKey);
	});
	var param = $.extend({typeKeyArr:typeKeyArr.join(',')}, obj.param || {});
	$.post(obj.url, param, function(data){
       obj.dictArr.forEach(function(dicType){
    	   var selectObj = $(dicType.el);
    	   if(dicType.addBlank === true){
    		   $("<option></option>").val('').text('').appendTo(selectObj);
    	   }
    	   data.rows.forEach(function(dict){
    		   if(dicType.typeKey){
    			   if(dict.typeKey == dicType.typeKey){
    				   $("<option></option>").val(dict[obj.valueKey || 'valueKey']).text(dict[obj.textKey || 'valueName']).appendTo(selectObj); 
    			   }
    		   }else{
    			   $("<option></option>").val(dict[obj.valueKey || 'valueKey']).text(dict[obj.textKey || 'valueName']).appendTo(selectObj);
    		   }
           });
    	   if(dicType.defaultVal){
    		   selectObj.val(dicType.defaultVal);
    	   }
	   });
    });
}
 ...
 * }]
 */
HtUtil.loadComboBoxByDict = function(obj){
	HtUtil.loadComboBox({
		url :  __ctx + "/system/dataDict/loadDataDict",
		dictArr : obj.dictArr
	});
}
/**
 * 加载数据字典下拉框信息---MM_PUB_DATA_DICT表
 * dictArr:[{
		typeKey:'YES_NO', -- 必须
		el : '#searchFormCol4', --必须
		defaultVal : '', --可选
 ...
 * }]
 */
HtUtil.loadComboBoxJsonByDict = function(obj){
	HtUtil.loadComboBox({
		url :  __ctx + "/system/dataDict/loadDataDictJson",
		dictArr : obj.dictArr
	});
}
