 ...
 * remark
 */
function goToContinueStep(wizard, options, state)
{
	toContinueStep();
}
...
function paginationClickHandler(event)
{
    ...
    switch (href.substring(href.lastIndexOf("#")))
    {
        ...
        case "#previous":
            ...
            break;
        case "#continue":
            goToContinueStep(wizard, options, state);
            break;
    }
}
...
function refreshPagination(wizard, options, state)
{
    if (options.enablePagination)
    {
        ...
        if (!options.forceMoveForward)
        {
            ...
            previous._hideAria();//mark  不提供上一步操作
        }
        if(state.currentIndex>1){//reamrk
        	var tocontinue = wizard.find(".actions a[href$='#continue']").parent();
        	tocontinue._hideAria();
        }
...
function renderPagination(wizard, options, state)
{
    if (options.enablePagination)
    {
        ...
        if(state.currentIndex<3){
	        buttons += format(buttonTemplate, "continue", "继续添加");
	    }

        if (!options.forceMoveForward)
        {
            buttons += format(buttonTemplate, "previous", "上一步");
        }

        buttons += format(buttonTemplate, "next", "下一步");

        if (options.enableFinishButton)
        {
            buttons += format(buttonTemplate, "finish", "完成");
        }
...
function renderTitle(wizard, options, state, header, index)
{
    ...
    stepItem.children("a").bind("click" + getEventNamespace(wizard), beforeClickHandler);//mark
}
 ...
 * remark
 */
function beforeClickHandler(event){
	if(event.bubbles){//过滤点击走过的步骤
		return ;
	}else{
		 event.preventDefault();
		 var anchor = $(this),
		    ...
		    isNotDisCur = anchor.parent().is(":not(.disabled):not(.current)");
	     formToStep(anchor,state.currentIndex,isNotDisCur);
	}
}
 ...
 * 保存成功后的回调函数
 */
$.fn.stepClickHandler = function(stepAnchor,isNotDisCur)
{
    var wizard = stepAnchor.parent().parent().parent().parent(),
        options = getOptions(wizard),
        state = getState(wizard),
        oldIndex = state.currentIndex;
 ...
 * remark
 */
$.fn.steps.toContinue = function ()
{
    var options = getOptions(this),
        state = getState(this);

    return goToContinueStep(this, options, state);
};
