...
app.controller("CustBtnController",['$scope','baseService','dragService','$http','$timeout',function($scope,baseService,dragService,$http,$timeout){
	$scope.custBtn={name:"选择",custQueryList:[],custDialog:{conditions:[]}};  //页面数据格式
	$scope.custQueryList = [];
	$scope.allFields=[];
	...
	$scope.getHtml = function(){
		...
		var custDialogConf = " ht-custDialog='"+JSON.stringify($scope.custBtn)+"' permission=\"permission.fields."+$scope.custBtn.permission+"\" ";
		
		if($scope.custBtn.custQueryList.length && $scope.custBtn.custDialog.alias == null){
			custDialogConf = "ng-show='false'" + custDialogConf;
		}
		if(formType=='mobile'){
	...
	$scope.innit = function(json){
		...
		$scope.$watch("loadStep",function(newValue, oldValue){
			...
			$timeout(function(){
				...
				$scope.setTreeTarget("custDialogTree",$scope.custBtn.custDialog);
				for(var i=0,query;query=$scope.custBtn.custQueryList[i++];){
	...
	$scope.setTreeTarget = function(treeId,json){
		...
		if("custDialogTree"==treeId &&  $scope.custBtn.custDialog.alias==null ) return;
	...
	$scope.changeCustDialog = function(ztreeId,isInnit){
		...
		if($scope.custBtn.custDialog.alias==null){
			$scope.custBtn.custDialog.conditions = [];
			return ;
		}
		for(var i=0,d;d=$scope.customDialogs[i++];){
			if(d.alias ==$scope.custBtn.custDialog.alias){
				...
				if(d.listDialog && d.listDialog.resultfield){
					...
					$scope.custBtn.custDialog.type="combiDialog";
				}
				if(d.resultfield){
					treeData =eval("(" + d.resultfield + ")");
					$scope.custBtn.custDialog.type="custDialog";
				}
	...
	$scope.getAllFields = function(){
		var post =  baseService.postForm(__ctx+"/form/form/getFormBoLists",{defId:defId});
	...
	$scope.getAllDialogs = function(){
		 baseService.get(__ctx +'/form/customDialog/getAllDialogs').then(function(data, status, headers, config){
