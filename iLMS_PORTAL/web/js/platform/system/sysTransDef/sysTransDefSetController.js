...
app.controller("SetController",['$scope','baseService','SysTransDef','ArrayToolService',function($scope,BaseService,SysTransDef,ArrayToolService){
	...
	$scope.excuteUpdateSql=function(){
		...
		if(id==""||!id){
			$.topCall.warn("请选择权限转移的类型(左边的树)", "提示信息");
			return;
		}
		if(param.authorId==""||!param.authorId){
			$.topCall.warn("请选择授权人", "提示信息");
			return;
		}
		if(param.targetPersonId==""||!param.targetPersonId){
			$.topCall.warn("请选择转移到的目标人", "提示信息");
			return;
		}
		if(param.targetPersonId==param.authorId){
			$.topCall.warn("授权人跟目标人不能为同一个人", "提示信息");
			return;
		}
		...
		SysTransDef.excuteUpdateSql(param,function(data){
			if(data.result==1){
				$.topCall.success(data.message,"操作成功",function(){
					$scope.authorChange();
				});
			}else{
				$.topCall.error(data.message,"操作失败");
			}
		});
	}
}]);