...
/*		tags+='<div id="webSignButtonSeal" style="'+this.configStyle.buttonStyle+'" >';
	...
	//	alert($('#'+this.divId).html());
		...
		if(!this.webSignObj||'undefined' == typeof (this.webSignObj.StatusCode)){
			$.topCall.warn("不能装载WEB印章控件，必须使用IE内核浏览器。可能需要在浏览器的Internet选项安全设置中修改ActiveX配置。",'提示信息',function(){
				window.history.back();
			});
			return false;
		}		
		...
		//	alert("X:"+arrys[0]+"   Y:"+arrys[1]);			
			...
			DialogUtil.open({
				...
				sucCall:function(retVal){
					...
					var signUrl=__ctx + "/platform/system/file/getFileById.ht?fileId=" + retVal.fileId;
					try{
