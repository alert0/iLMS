...
(function($) {
	...
	$.fn.rowOps = function() {
		var me = $(this),
			...
			widthed = 0;
		if(num<1){//少于N个按钮，直接显示，不隐藏
			me.css('width',num*40);//每个按钮长度60px
			var aa =  me.children("a")[0];
			if(aa != undefined)
				$(aa).addClass("ops_single");
			return;
		}
		...
		me.find("a").each(function(i,n) {
			...
			if (typeof isCommon == "undefined" || isCommon != "true") {
				...
				//字的宽度+偏移的宽度+图标的宽度
				widthed +=  (textLength+15+26);	//5 - >15

				if(i+1==num){
					ops_ul.css("width",widthed);
					var left=parseInt( "-" + widthed);
					ops_ul.parents(".ops_box").css({"left":left});
				}
			} else {
				hasCommon = true;
			}
...
$(function(){
	$.extend({initRowOps:function(){
		/** 初始化 */
		try{
			$("div[class$='colManage']").each(function() {
				$(this).rowOps();
			});
