...
public class SwLongOrderController extends GenericController{
	...
	public @ResponseBody PageJson queryDemandOrderPage(HttpServletRequest request, HttpServletResponse reponse,
		...
		if (user.getUserType() == IUser.USER_TYPE_SUPP) {
	...
	public void downloadLongOrderByBatch(HttpServletRequest request, HttpServletResponse response,
			 SwLongOrderModel model) throws Exception {

		try {
			String longOrderType = RequestUtil.getString(request, "longOrderType");
			...
			if (null != list) {	
				if ("formal".equals(longOrderType)) {
					String exportFileName = "长周期订单正式需求导出" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
					String[] headers = { "版本号", "订单号", "发布月","车型", 
							"零件号", "简号", "零件名称", "订单月","订购数量", "收货数量","收货状态",
							"供应商代码", "出货地","供应商名称","单位" };
					String[] columns = { "version", "orderNo", "currentMonth","carType", 
							"partNo","partShortNo","partNameCn",  "objMonth" ,"orderNum","receiveQty","receiveStatus",
							"supplierNo", "supFactory", "supplierName","partUnit" };
					int[] widths = { 100, 100, 80, 80, 80, 80, 80, 80, 80,80,
							80, 80, 150, 200,80 };
					ExcelUtil.exportExcel(ExcelUtil.EXCEL_XLSX, request, response, exportFileName, list, headers,
								widths, columns);
				}
				if ("forecast".equals(longOrderType)) {
					String exportFileName = "长周期订单预测需求导出" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
					String[] headers = {"版本", "零件号", "简号", "零件名称",
							"预测月1",
							"预测月1数量", "预测月2", "预测月2数量", "预测月3", "预测月3数量",
							"供应商代码", "出货地","供应商名称","单位" };
					String[] columns = { "version","partNo","partShortNo","partNameCn",  
							"forecastFirst",
							"forecastFirstNum", "forecastSecond", "forecastSecondNum", "forecastThird", "forecastThirdNum",
							"supplierNo", "supFactory", "supplierName","partUnit" };
					int[] widths = { 80,130, 80, 150, 
							 80, 80, 80, 80, 80, 80, 
							80, 80, 150, 80 };
					ExcelUtil.exportExcel(ExcelUtil.EXCEL_XLSX, request, response, exportFileName, list, headers,
								widths, columns);
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/downloadLongOrderByBatchFormal")
	public void downloadLongOrderByBatchFormal(HttpServletRequest request, HttpServletResponse response,
			@ModelAttribute("model") SwLongOrderModel model) throws Exception {
		...
		try {
			...
			if (null != list) {				
				String[] headers = { "版本号", "订单号", "发布月","车型", 
						...
						"收货数量"};
				String[] columns = { "version", "orderNo", "currentMonth","carType", 
						"partNo","partShortNo","partNameCn",  "objMonth" ,"orderNum","receiveQty"};
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/downloadLongOrderByChooseFormal")
	public void downloadLongOrderByChooseFormal(HttpServletRequest request, HttpServletResponse response) throws Exception {
		 String longOrders = RequestUtil.getString(request, "longOrderArr");
		try {
			String exportFileName = "长周期订单" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/downloadLongOrderByBatchForecast")
	public void downloadLongOrderByBatchForecast(HttpServletRequest request, HttpServletResponse response,
			@ModelAttribute("model") SwLongOrderModel model) throws Exception {
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/downloadLongOrderByChooseForecast")
	public void downloadLongOrderByChooseForecast(HttpServletRequest request, HttpServletResponse response) throws Exception {
		 String longOrders = RequestUtil.getString(request, "longOrderArr");
		try {
			String exportFileName = "长周期订单" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
...
//	} 
	 ...
	 * @throws  
	 * @author luoxq
	 * @date 2019年3月23日 下午12:07:52
	 */
	@RequestMapping("updateFeedbackLongOrder")
	public void updateFeedbackLongOrder(HttpServletRequest request,HttpServletResponse response, 
			@RequestBody List<SwLongOrderModel> list) 
			throws Exception{
			ResultMessage message=null;
//			String idArr = null;
			String feedbackStatus = RequestUtil.getString(request, "feedbackStatus");
			String feedbackRemark = RequestUtil.getString(request, "feedbackRemark");
			SwLongOrderModel model = new SwLongOrderModel();
			try {
				if ( null != feedbackStatus) {
					model.setFeedbackRemark(feedbackRemark);
					model.setFeedbackStatus(feedbackStatus);
					manager.updateFeedbackLongOrder(list,model);
				}
	 ...
	 * @throws  
	 * @author luoxq
	 * @date 2019年4月10日 上午11:34:28
	 */
	@RequestMapping("deleteLongOrder")
	public void deleteLongOrder(HttpServletRequest request,HttpServletResponse response, 
			 SwLongOrderModel model) 
			throws Exception{
			ResultMessage message=null;

			try {
				manager.deleteLongOrder(model,RequestUtil.getIpAddr(request));
	 ...
	 * @throws  
	 * @author luoxq
	 * @date 2019年4月21日 下午6:12:54
	 */
	@RequestMapping("updateLongReceiveQty")
	public void updateLongReceiveQty(HttpServletRequest request,HttpServletResponse response, 
			 SwLongOrderModel model) 
			throws Exception{
			ResultMessage message=null;

			try {
				manager.updateLongReceiveQty(model,RequestUtil.getIpAddr(request));
	...
	public @ResponseBody SwLongOrderModel getDefaultVersion(HttpServletRequest request,HttpServletResponse response) throws Exception {
		try {
			IUser user = ContextUtil.getCurrentUser();
			Map<String, String> map = new HashMap<String, String>();
			map.put("factoryCode", user.getCurFactoryCode());
			SwLongOrderModel model = manager.getDefaultVersion(map);
