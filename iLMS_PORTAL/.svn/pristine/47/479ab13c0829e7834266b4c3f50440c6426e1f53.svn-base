...
@Service("SwVentureForecastWeekManager")
public class SwVentureForecastWeekManagerImpl extends AbstractManagerImpl<String, SwVentureForecastModel>
implements SwVentureForecastWeekManager{

	@Resource
	private SwVentureForecastWeekDao dao;
...
//			model.setEndDate(swVentureForecastModel.getEndDate());
	...
	public void checkDataRsult(SwVentureForecastModel model, Page page) {
		//查询要发布版本的对象周（月）、发布周（月）、开始日期、结束日期
		SwVentureForecastModel swVentureForecastModel = dao.selectDemandForecasetByTagVersion(model); 
		model.setObjWeek(swVentureForecastModel.getObjWeek().substring(0,10));
...
//		return dao.checkDataRsult(model,page);
	...
	public PageList<Map<String, Object>> queryVentureForecastWeekExportDataByPage(String startMonthStr,
		...
		return dao.queryVentureForecastWeekExportDataByPage(startMonthStr,endMonthStr,model,page);
	}
