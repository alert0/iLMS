...
function changeScrollHeight(noNeedFoldHeight,time){
	...
	window.setTimeout(function(){
		...
		//default样式下顶部menu高度
		var defaultMenuHeight = $(".index_menu",$(parent.document)).height()/2||0;
		if(defaultMenuHeight != 0){
			tableBody.height($(window).height()-tableBody[0].offsetTop-defaultMenuHeight-foldHeight-moreHeight-51);
			// 设置蒙版高度
			$(".bgc").height($(window).height()-tableBody[0].offsetTop-defaultMenuHeight-foldHeight-moreHeight);
		}else{
			//extendIndex样式下顶部menu高度
			var extendIndexMenuHeight = $(".menu",$(parent.document)).height()||0;
			tableBody.height($(window).height()-tableBody[0].offsetTop-extendIndexMenuHeight-foldHeight-moreHeight-51);
			// 设置蒙版高度
			$(".bgc").height($(window).height()-tableBody[0].offsetTop-extendIndexMenuHeight-foldHeight-moreHeight);
		}
		tableBody.css("overflow-y","scroll");
		tableBody.css("border","1px solid #cddce6");
