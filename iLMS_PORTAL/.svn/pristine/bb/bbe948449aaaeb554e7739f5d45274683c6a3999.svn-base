...
<script>
	...
	function loadGrid(orderNo){
		$('#grid_detail').datagrid($.extend($defaultOptions,{
			url :  __ctx + "/jiso/order/queryJisoOrderDetailPageByOrderNo?orderNo=" + orderNo,
			striped : true,
			fitColumns : false,
			columns : [ [
				...
				{field : 'arriveStatus',sortName : "arriveStatus",title : '收货状态',width : 130,align : 'center',sortable : 'true'} 
			 ] ]
		}));
	}
