...
public class SwNonStandardController extends GenericController {
	...
	private String FTP_PATH = "d:/ilms/ftp/";
	private String LOCAL_PATH = "c:/ILMS/SPE/";
...
//					String tagImgPath = imgLastPath + imgName;
					String fileImgPath = LOCAL_PATH + user.getAccount() + "/" ;
...
//					nse.saveToFile(list.get(i).getFeatureValue(), tagImgPath);
					...
					if (files != null && files.length > 0) { // 此方法判断OK,需要使用数组的长度来判断。
//						SwNonStandardController.delAllFile(imgPath);  // 如文件不为空则删除
						SwNonStandardController.delAllFile(fileImgPath); 
					} else {  //如果文件夹不存在则创建
						file.mkdirs();
					}
				} else {
					...
					String  tagImgPath = fileImgPath+ fileId + picSuffix;   //本地硬盘路径，可配置成系统参数
					nse.saveToFile(list.get(i).getFeatureValue(), tagImgPath);  //图片下载到服务器硬盘
//					String tagUrl = "http://"  + ipAddr + ":" + port + "/css/image/img/"+ fileName+ "/"  +imgName;
					String strImg = nse.GetImageStr(tagImgPath);
					if (".svg".equals(picSuffix) || ".SVG".equals(picSuffix)) {   //如果是svg格式，则转换为png
						String svgToPngPath = fileImgPath+  fileId  + ".png";
						NonStandExportUtil.convertToPng(strImg, svgToPngPath);
						tagImgPath = tagImgPath.substring(0,tagImgPath.lastIndexOf('.')) + ".png";
					}
//					FtpFileUtil f = new FtpFileUtil("172.18.189.115", 21, "admin115", "admin");  //配置的ftp服务器地址端口账号和密码，在系统参数中获取
//					
					FtpFileUtil f = new FtpFileUtil();
					FileInputStream srcFile = new FileInputStream(new File(tagImgPath));
					DefaultFile file2 = new DefaultFile();
					/**
					 * 本地图片路径
					 * ftp服务器存储图片路径
					 * ftp服务器存储图片名称
					 */
					String ftpPictureName = fileId + picSuffix;
//					String path = "/lxq/test";
					String path = "/SPE"+"/"+user.getAccount() ;
					//文件保存到FTP服务器
					file2.setStoreType(DefaultFile.FILE_STORE_FTP);
					boolean g = f.upLoadFile(srcFile, path, ftpPictureName);
					if (g) {
						
						// 向数据库中添加数据
						file2.setId(fileId);
						// 附件名称
						file2.setFileName(imgName.substring(0, imgName.lastIndexOf('.')));
						file2.setFilePath(path);  //保存相对路径
						// 上传时间
						file2.setCreateTime(new java.util.Date());
						// 扩展名
						file2.setExt(picSuffix.substring(picSuffix.indexOf('.')+1));
						// 字节总数
//						file2.setByteCount(f.getSize());
						// 说明
//						file2.setNote(FileUtil.getSize(f.getSize()));
						// 总的字节数
						file2.setIsDel(DefaultFile.FILE_NOT_DEL);
						// 当前用户的信息
						if (user != null) {
							file2.setCreator(user.getFullname());
							file2.setCreateOrgId(user.getUserId());
						} else {
							file2.setCreatorName(DefaultFile.FILE_UPLOAD_UNKNOWN);
						}
					
						fileManager.create(file2);
					}
					list.get(i).setFeatureValue(fileId);
				}
	...
	//param path 文件夹完整绝对路径
	   public static boolean delAllFile(String path) {
	   	...
	   	public static void main(String[] args) {
	   		try {
	   			File file = new File("d:/ilms/ftp/");
				
				if (file != null && !file.exists() && !file.isDirectory()) {
					file.mkdirs();
				}
	   			NonStandExportUtil nse = new NonStandExportUtil();
	   			nse.saveToFile("http://172.18.189.62:9003/iLMS_SERVICE/img/100000342-B86.svg", "d:/ilms/ftp/100000342-B86.svg");
	   			
//				FtpFileUtil f = new FtpFileUtil("10.88.52.8", 21, "admin", "admin@2019");
//				
//				FileInputStream srcFile = new FileInputStream(new File("e:\\test/[销售订单号100000342][B86]刺绣.png"));
//				boolean g = f.upLoadFile(srcFile, "iLMS_FILE/lxq/test", "abc1235.jpg");
//				System.out.println("upload:" + g);
				
//				FileOutputStream fos = new FileOutputStream(new File("e:\\test/ftpdown中文jpg.jpg"));
//				Boolean d = f.downLoadFile(fos, "admin/test/tes", "abc123.jpg");
//				System.out.println("download:"+d);
				
//				FileInputStream srcFile2 = new FileInputStream(new File("e:\\test/tesjpg.jpg"));
