	...
	$.extend($.fn, {
		...
		validRules : function(value,validRule,element,$dirty){
			...
			for (var i=0,v;v=validates[i++];) {
				...
				errMsg += _valid.errMsg;
				//如果当前规则验证不通过则直接返回false
				if(!_valid._valid) return {"_valid":false,"errMsg":_valid.errMsg};
			}
			//全部验证通过则返回true
			...
			"daterangestart":{
					rule : function(v, b,e) {
						if(!$(e).data("iscompile")){
							...
							var compile = AngularUtil.getService("$compile");
							compile($("[ng-model='"+b.target+"']"))(scope);
							$("[ng-model='"+b.target+"']").data("iscompile",true);
						} else {
							$(e).data("iscompile", false);
						}
