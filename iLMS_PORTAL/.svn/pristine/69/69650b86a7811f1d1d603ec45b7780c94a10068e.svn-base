...
<body>
	<div class="easyui-layout" fit="true" scroll="no">
		<div data-options="region:'center',border:false" style="text-align: center;">
			...
			<%-- 	<div title="组织简介">
					<iframe data-options="closable:false"  id="listFrame" src="${ctx}/org/org/orgGet?id=${param.id}&demId=${param.demId}" frameborder="no" width="100%" height="100%"></iframe>
				</div> --%>
				<div title="组织人员" style="overflow: auto; padding: 0; margin: 0">
					<iframe id="listFrame2" src="${ctx}/business/addressBook/addressBook/userList?orgId=${param.id}&demId=${param.demId}" frameborder="no" width="100%" height="100%"></iframe>
				</div>
				<%-- <div title="组织岗位" data-options="closable:false" style="overflow: auto">
					<iframe id="listFrame3" src="${ctx}/org/orgRel/orgRelList?orgId=${param.id}" frameborder="no" width="100%" height="100%"></iframe>
				</div>
				<div title="组织参数" data-options="closable:false" style="overflow: auto">
					<iframe id="listFrame4" src="${ctx}/system/sysUserParams/sysUserParamsEdit?type=2&id=${param.id}" frameborder="no" width="100%" height="100%"></iframe>
				</div>
				<div title="组织分级管理设置" data-options="closable:false" style="overflow: auto">
					<iframe id="listFrame5" src="${ctx}/org/orgAuth/orgAuthList?demId=${param.demId}&orgId=${param.id}" frameborder="no" width="100%" height="100%"></iframe>
				</div> --%>
			</div>
