	...
	$(function(){
		...
		$(".fa-save").click(function() {
			...
			if(dicIds.length>1){
				...
				var url="${ctx}/system/dataDict/sort";
				var params={"dicIds":dicIds};
	 			$.post(url,params,function(result){
	 				var obj=new com.hotent.form.ResultMessage(result);
	 				if(obj.isSuccess()){//成功
	 					$.topCall.success("排序完成!",function(){
	 						window.parentWindow.loadDictByTypeId();
	 						window.selfDialog.dialog('close');
	 					},'提示信息');	 						
	 				}
	 				else{
	 					$.topCall.error('出错信息',"排序失败",obj.getMessage());
	 				}
	 			});
			}
		});
	...
	<form id="dataForm" method="post" action="sort">
		<div class="form-table">
			<table class="table-detail" border="0" cellspacing="0"
				cellpadding="0">
				<tr>
					<td class="form_title" align="center" style="padding-left: 50px"><select class="inputText" id="dicIds"
						name="dicIds" size="12" style="width: 100%;" multiple="multiple">
							<c:forEach items="${dataDictList}" var="d">
