...
(function() {
	...
	Object.extend = function(destination, source) {
		for ( var property in source) {
			destination[property] = source[property]; // 利用动态语言的特性,
														// 通过赋值动态添加属性与方法
		}
	...
	Object.extend(Object, {
		...
		toInspect : function(object) {
			try {
				if (object == undefined)
					return 'undefined'; // 处理undefined情况
				if (object == null)
	...
	//是否为日期2015-1-01的字符串形式
	function isDateStr(dateStr) {
	        if (!isNaN(dateStr) && dateStr.toString().length == 13)return false;
	...
	Object.extend(Object, {
		...
		//add by zyg ：2016-3-31
		formatItemValue:_formatItemValue,
		toIsJSON : _IsJSON,
...
Object.extend(Date, {
	...
	function getDatePeriodDay(startDate, endDate) {
		return (Math.abs(startDate * 1 - endDate * 1) / 60 / 60 / 1000 / 24)
				.toFloor();
	}
});
				...
				(function() {
					...
					function jsonStringToDate(string) {
						...
						if (match = string.match(R_ISO8601_STR)) {
							...
							dateSetter.call(date, toInt(match[1]),
									toInt(match[2]) - 1, toInt(match[3]))
							var h = toInt(match[4] || 0) - tzHour
							var m = toInt(match[5] || 0) - tzMin
							var s = toInt(match[6] || 0)
							var ms = Math.round(parseFloat('0.'
									+ (match[7] || 0)) * 1000)
...
Object.extend(Function.prototype,
		(function() {
			...
			function bind(context) {
				if (arguments.length < 2 && Object.toIsUndefined(arguments[0]))
					return this;
...
function _StringStartWith(startStr, ignorecase) {
	...
	return ignorecase ? startString.toLocaleLowerCase() == startStr
			.toLocaleLowerCase() : startString == startStr;
}
...
function _StringEndWith(endStr, ignorecase) {
	...
	return ignorecase ? endString.toLocaleLowerCase() == endStr
			.toLocaleLowerCase() : endString == endStr;
...
function _Truncate(length, truncation) {
	...
	return this.length > length ? this.slice(0, length) + truncation
			: String(this);
}
 ...
 * @returns {String}
 */
function _formatItemValue(arr,value){
	var result="";
...
function _GetGuid(isMiddline) {
	...
	return (S4() + S4() + charStr + S4() + charStr + S4() + charStr + S4()
			+ charStr + S4() + S4() + S4());
}
...
function _UnescapeHTML(object) {
	return object._StringtrimTags.replace(/&lt;/g, '<').replace(/&gt;/g, '>')
			.replace(/&amp;/g, '&');
}
// 编码为中文符号：<转为〈
...
function _DateObjectFormat(dateObject, format) {
	...
	if (/(y+)/.test(format))
		format = format.replace(RegExp.$1, (dateObject.getFullYear() + "")
				.substr(4 - RegExp.$1.length));
	for ( var k in o)
		if (new RegExp("(" + k + ")").test(format))
			format = format.replace(RegExp.$1, RegExp.$1.length == 1 ? o[k]
...
/*
 * 获得时间差,时间格式为 年-月-日 小时:分钟:秒 或者 年/月/日 小时：分钟：秒 其中，年月日为全格式，例如 ： 2010-10-12
 * 01:00:00 返回精度为：秒，分，小时，天 diffType：day，minute，hour，second var result =
...
function _GetDateDiff(startTime, endTime, diffType) {
	...
	var subdif = parseFloat((eTime.getTime() - sTime.getTime())
			/ parseInt(divNum));
...
function _ajaxJson(url, method, data, async, type, callback, dataType) {
	...
	$.ajax({
		...
		error : function(d) {
			if (typeof (d) == "object") {
				throw new Error("服务器返回数据出错" + JSON.stringify(d));
...
function _ajaxDownloadFile(url, method, data) {
	...
	var form_export = $("<form>").attr("action", url)
			.attr("target", iframeName).attr("method", "POST");
	// 构造参数
	for ( var i in theData) {
		form_export.append($("<input>").attr("name", theData[i].name).val(
				theData[i].value));
	}
	var ifrme = $("<iframe>").attr("name", iframeName).css({
		display : 'none'
	});
	div.append(ifrme);
	div.append(form_export).css({
		display : 'none'
	});
	$("body").append(div);
...
function showtoModalDialog(url, dlgWidth, dlgHeight, objectParm) {
	...
	var form = "resizable:yes;scroll:no;status:no;dialogHeight:" + dlgHeight
			+ "px;dialogWidth:" + widthTmp + "px;dialogLeft:" + dlgLeft
			+ ";dialogTop:" + dlgTop;
...
function _showLoading(message) {
	...
	var loadingHtml = "<div id=\"LoadingDiv\" style='text-align:center; border:solid 2px #D4F0FC; font-weight: bold; float: left; width: 100%;  height:100%;line-height: 100%;  position: absolute;  top: 1px; left: 1px; z-index: 1000;"
			+ opacity
			+ "\" style=\"text-align: center; background-color:#F6F6F6;  font-size: 14px; color: Red; padding-top:"
			+ paddingtop
			+ "px;'><div><img alt=\"\" src=\""
			+ ui
			+ "/js/lib/easyui/themes/default/images/loading.gif\" /></div><div>"
			+ msg
			+ "<span style='cursor:pointer' onclick=\"_closeLoading();\">关闭</span></div></div>";
	if ($("body")) {
		if ($("#LoadingDiv")[0]) {
			$("#LoadingDiv").show();
		} else {
