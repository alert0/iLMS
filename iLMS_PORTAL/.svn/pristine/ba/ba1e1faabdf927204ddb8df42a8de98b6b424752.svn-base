...
WebSignPlugin = {
		...
		init:function(){
			...
			if(this.fileObj.length==0) return;
			
			this.user = ShareFunction.getCurUser();
			
			var name=this.fileObj.attr("name")||this.fileObj.attr("ng-model");
			var fileId=this.fileObj.val();				
			//容器的ID
			var divId="div_" + name.replaceAll("\\.","_");
			
			var right=this.fileObj.attr("right");			//没有权限，删除div容器。
			if(right=="no"){
				$("#" + divId).remove();
			}else{             //有读和写的权限，加载控件。
//					$.topCall.waitting('正在加载印章文档,请稍候...');
				//加载控件。
...
//					$.topCall.closeWaitting(); 
		...
		submit:function(){
			...
			if(right!="no"){
	            ...
	            $.topCall.error("没有权限修改Web印章内容!",'提示信息');
		    }
		}
};
...
function getntkoWebSignObjX(){
	...
	if(WebSignPlugin.fileObj==null||'undefined'== typeof (WebSignPlugin.fileObj)||WebSignPlugin.fileObj.length<1){   
		$.topCall.error("请添加Web印章控件,再进行操作!(控件只支持IE内核的浏览器)",'提示信息');
		return null;
	}
