			...
			<div id="gridSearch" class="toolbar-search ">
				<div class="toolbar-head">
					<div class="buttons">
						...
						<a class="btn btn-primary btn-sm fa-plus" href="javaScript:void(0)" onClick='openDetail("","add")'>
							<span>添加</span>
						</a>
						<a class="btn btn-primary btn-sm fa-remove" href="javaScript:void(0)" action="/calendar/shift/remove">
							<span>删除</span>
						</a>
				...
				<div class="toolbar-body">
					<form id="searchForm" class="search-form">
						<ul>
							<li><span>班次名称:</span><input type="text" class="inputText"
								name="Q^name_^SL"></li>
						</ul>
					</form>
				</div>
	...
	<script type="text/javascript">
		function loadGrid() {
			$('#grid').datagrid($.extend($defaultOptions,{
				url : __ctx + "/calendar/shift/listJson",
				idField : "id",
				sortName : 'id_',
				sortOrder : 'name',
				columns : [ [ 
				    ...
				    {field : 'minutes',sortName : "minutes_",title : '工时（分钟）',width : 150,align : 'left',sortable : 'true'},
		...
		function openDetail(id, action) {
			var title = action == "edit" ? "编辑班次" : action == "add" ? "添加班次" : "查看班次";
			var url = "shiftEdit";
			if (id && id != "") {
				url += "?id=" + id;
			}
		...
		function mangerFormater(value, row, index){
			var result = "";
			if (row.isDefault != '1') {
				result += '<a class="btn btn-default fa fa-send" href="${ctx}/calendar/shift/setDefault?id='+row.id+'">设为默认</a>';
			}
			result += '<a class="btn btn-default fa fa-edit" ref="javascript:;" onclick="openDetail(\''+row.id+'\',\'edit\')">编辑</a>';
			return result;
		}
