				...
				<div class="toolbar-box border">
					<div class="toolbar-head">
						...
						<div class="buttons">
							...
							<a class="btn btn-sm btn-primary fa-remove" href="javascript:;" action="/base/base/relResource/remove">
								<span>删除</span>
							</a>
						</div>
						<div class="tools">
					...
					<div class="toolbar-body">
						<form id="searchForm" class="search-form">
							<ul style="margin-left: -26px">
								<li><span>资源ID:</span><input class="inputText" type="text" name="Q^RES_ID_^SL"></li>
								<li><span>名称:</span><input class="inputText" type="text" name="Q^NAME_^SL"></li>
								<li><span>资源地址:</span><input class="inputText" type="text" name="Q^RES_URL_^SL"></li>
							</ul>
						</form>
					</div>
				</div>
...
<script>
	...
	function openDetail(id, action) {
		var title = action == "edit" ? "编辑关联资源" : action == "add" ? "添加关联资源" : "查看关联资源";
		action =action == "edit" ? "Edit" : action == "add" ? "Edit" : "Get";
		var url="relResource" + action;
		if(!$.isEmpty(id)){
			url+='?id=' + id;
		}
	...
	function loadGrid() {
		$('#grid').datagrid($.extend($defaultOptions,{
			...
			striped : true,
			columns : [ [
			{field : 'resId',sortName : "RES_ID_",title : '资源ID',width : 250,align : 'center',sortable : 'true'
			...
			{field : 'resUrl',sortName : "RES_URL_",title : '资源地址',width : 250,align : 'center',sortable : 'true'
			...
			}, 
			{
				...
				formatter : function(value, row, index) {
					...
					result += "<a class='rowInLine btn btn-default fa fa-remove' action='/base/base/relResource/remove?id=" + row.id + "' herf='#'>删除</a>";
					return result;
				}
			} ] ],
