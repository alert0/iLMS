...
//				message = new ResultMessage(ResultMessage.FAIL, "反馈失败");
	...
	public void downloadSwDemandForecastModel(HttpServletRequest request,HttpServletResponse response, 
		...
		String[] headers = {"订单物流模式", "区域",  "车型", "集货路线", "累计车次", "合并车次","出货地","供应商代码",
		...
		String[] columns = {"logisticsMode","area", "carType","routeCode","totalBatchs", "mergeBatchs",
				   			"supFactory","supplierNo","supplierName","orderNo","purchaseNo","workDate","todayCarBatch","planPickupTime",
...
//			ExcelUtil.exportException(e, request, response);
	...
	public @ResponseBody PageJson queryOrderDetail(HttpServletRequest request,HttpServletResponse response,
		...
		IUser user = ContextUtil.getCurrentUser();
		modle.setFactoryCode(user.getCurFactoryCode());
	...
	public void exportExcelForOrderDetails(HttpServletRequest request,HttpServletResponse response)throws Exception {
		...
		try {
			...
			String[] headers = {"订单号","区域","取货路线","车型","计划取货时间","计划到货时间","订购日期",
								"订单类型","订购单位","零件编号","简号","零件名称","供应商代码","供应商名称","出货地代码",
								"出货地地址","是否紧急订单","仓库代码",
								"订购数量","取消数量","已发数量","待发数量","收货数量","首次发布时间","最后修改时间"};
			String[] columns = {"purchaseNo","area","routeCode","carType","planPickupTime","planArrTime","arriveDate",
								"orderType","orderUnit","partNo","partShortNo","partNameCn","supplierNo","supplierName","supFactory",
								"supFactoryAddr","isUrgent","depotNo",
								"orderQty","cancelNum","totalDelivQty","waitQty","receiveCount","creationTime","lastModifiedTime"};
			int[] widths = {60,60,60,60, 85,85,80,60,100,100,60,150,60,200,60,200, 60,80,80,80,80,80,80,120,120};
