...
function downloadTemplateFn(){
	var downurl = encodeURI(encodeURI(__ctx + '/comm/comm/downloadExcelModel?fileName=供应商分组信息导入模板.xlsx'));
		...
		success : function(responseText) {
			...
			if (resultMessage.isSuccess()) {
				...
				$.topCall.confirm("提示",showMsg,
					function(r) {
						if (!r) return;
						$('#importForm').attr("action", __ctx + "/sw/swSupplierGroup/importModel");
						$('#importForm').submit();
					}
				);
			}
...
function isImportFn(){
	...
	var url=__ctx + "/sw/swSupplierGroup/isImport";
	console.log(98989);
...
function downloadDataFn(){
	var downurl = encodeURI(__ctx + '/sw/swSupplierGroup/exportTempData');
	$('#downloadiframe').attr('src', downurl);
}
	
function loadGrid() {
	$('#grid').datagrid($.extend($defaultOptions,{
		url :  __ctx + "/sw/swSupplierGroup/curdlistImportTempJson",
		idField : "groupId",
		columns : [ [
		{field : 'groupId',checkbox : true},     
		{field : 'groupName',title : '分组名称',width : 130,align : 'center'}, 
