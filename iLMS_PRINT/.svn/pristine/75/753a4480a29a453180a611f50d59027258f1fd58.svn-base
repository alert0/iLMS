...
public class printDome implements Printable {
	...
	public int print(Graphics graphics, PageFormat pageFormat, int pageIndex)
		...
		if (pageIndex == 0) {
			...
			graphics2d.setStroke(new BasicStroke(0.5f, BasicStroke.CAP_BUTT,
					BasicStroke.JOIN_MITER, 2.0f, dash, 0.0f)); // 字体高度
			...
			graphics2d.drawString(str, (float) x, (float) y + 1 * heigth
					+ img_Height);

			graphics2d.drawLine((int) x,
					(int) (y + 1 * heigth + img_Height + 10), (int) x + 200,
					(int) (y + 1 * heigth + img_Height + 10));

			graphics2d.drawImage(src, (int) x, (int) (y + 1 * heigth
					+ img_Height + 11), c);
			return PAGE_EXISTS;
	...
	private static javax.print.PrintService getPrinterByName(String printerName) {
	...
	public static void main(String[] args) {
		...
		book.append(new printDome(), format);
		...
		String printerName = "ITPrinter";
		javax.print.PrintService printer = getPrinterByName(printerName);
		
		// 设置打印类

		job.setPageable(book);
		PrinterStateReasons printerStateReasons = job.getPrintService().getAttribute(PrinterStateReasons.class);
